...
> [!DANGER] Problemi
>  Controllare che le formule siano giuste
...
Date tutte queste informazioni possiamo calcolare il **tempo di trasferimento** complessivo anche detto **throughput teorico**, definito come:
$$T_{\text{trasferimento}} = T_{\text{frame}} + T_{\text{propagazione}}$$
Per calcolare il **throughput reale** invece dobbiamo considerare anche il tempo di $ACK$ e il tempo di $RTT$(Round-Trip Time),quindi la formula diventa:
$$T_{\text{effettivo}} = T_{\text{frame}} + T_{\text{ACK}} + RTT + T_{\text{elaborazione}}$$
$$T_{ACK} = \frac{\text{Dimensione dell'ACK (in bit)}}{\text{Bandwidth (in bit/s)}}$$
In questo caso quando usiamo questa formula negli esercizi sia $RTT$ che $T_{\text{elaborazione}}$ lo da l'esercizio
![[Pasted image 20260319105431.png|500]]
Per aumentare il throughput reale si usano diverse modalità tra cui:
